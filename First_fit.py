...
class FirstFit:
    ...
    def allocate_process(self):
        ...
        target_block = self.find_first_fit(process_size)
        if target_block == -1:
    ...
    def find_first_fit(self, process_size: int) -> int:
        for i in range(len(self.memory_block)):
            if self.memory_block[i].size >= process_size and self.available_block[i]:
                self.available_block[i] = False
                return i
        return -1
